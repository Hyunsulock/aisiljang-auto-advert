...
export function setupPropertyOwnerHandlers() {
  /**
  ...
  ipcMain.handle('propertyOwner:setSession', async (_, accessToken: string, refreshToken: string) => {
    try {
      ...
      console.log('[PropertyOwnerHandlers] Session set from renderer');
      return { success: true };
    } catch (error: any) {
  ...
  ipcMain.handle('propertyOwner:get', async (_, propertyName: string, dong: string, ho: string) => {
    try {
      const properties = await propertyOwnerRepo.getPropertiesByKeys([
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        console.log('[PropertyOwnerHandlers] Saving property owner:', {
          propertyName,
          dong,
          ho,
          verificationInfo,
        ...
        console.log('[PropertyOwnerHandlers] Property path:', {
        ...
        if (documentFilePath) {
          ...
          console.log('[PropertyOwnerHandlers] Uploading document to:', fullPath);
          const uploadResult = await fileStorageService.uploadFile(
            documentFilePath,
            fullPath
          );
          documentStoragePath = uploadResult.path;
          console.log('[PropertyOwnerHandlers] Document uploaded:', documentStoragePath);
        }
        ...
        if (registerFilePath) {
          ...
          console.log('[PropertyOwnerHandlers] Uploading register to:', fullPath);
          const uploadResult = await fileStorageService.uploadFile(
            registerFilePath,
            fullPath
          );
          registerStoragePath = uploadResult.path;
          console.log('[PropertyOwnerHandlers] Register uploaded:', registerStoragePath);
        }
        ...
        if (powerOfAttorneyFilePath) {
          ...
          console.log('[PropertyOwnerHandlers] Uploading power of attorney to:', fullPath);
          const uploadResult = await fileStorageService.uploadFile(
            powerOfAttorneyFilePath,
            fullPath
          );
          powerOfAttorneyStoragePath = uploadResult.path;
          console.log('[PropertyOwnerHandlers] Power of attorney uploaded:', powerOfAttorneyStoragePath);
        }
        ...
        console.log('[PropertyOwnerHandlers] Data to save:', dataToSave);

        // Supabase에 저장
        const propertyWithVerification = await propertyOwnerRepo.savePropertyWithVerification(
          { name: propertyName, dong: dong || '', ho: ho || '' },
          dataToSave
        );

        console.log('[PropertyOwnerHandlers] Saved successfully:', propertyWithVerification);
    ...
    try {
      ...
      if (property) {
        ...
        if (property.document_file_path) {
          await fileStorageService.deleteFile(property.document_file_path);
        }
        if (property.register_file_path) {
          await fileStorageService.deleteFile(property.register_file_path);
        }
        if (property.power_of_attorney_file_path) {
          await fileStorageService.deleteFile(property.power_of_attorney_file_path);
        }

        // DB에서 삭제
        await propertyOwnerRepo.deleteVerificationInfo(property.id);
      }
  ...
  ipcMain.handle('propertyOwner:downloadFile', async (_, storageFilePath: string, saveAsName: string) => {
    try {
      ...
      await fileStorageService.downloadFile(storageFilePath, result.filePath);
  ...
  ipcMain.handle(
    ...
    async (_, propertyName: string, dong: string, ho: string, fileType: 'document' | 'register' | 'powerOfAttorney') => {
      try {
        ...
        // 스토리지에서 파일 삭제
        if (filePathToDelete) {
          await fileStorageService.deleteFile(filePathToDelete);
        }
        ...
        // DB 업데이트
        await propertyOwnerRepo.savePropertyWithVerification(
          { name: propertyName, dong: dong || '', ho: ho || '' },
          {
            agency_id: agencyId,
