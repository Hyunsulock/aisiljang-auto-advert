 ...
 * Agency 관리 모듈
 */
export class AgencyModule implements AppModule {
  constructor() {}

  async enable(_context: ModuleContext): Promise<void> {
    console.log('[AgencyModule] Initializing...');
    ...
    ipcMain.handle('agency:setSession', async (_, accessToken: string, refreshToken: string) => {
      try {
        console.log('[AgencyModule] Setting session in Main process');
        ...
        if (error) {
          console.error('[AgencyModule] Failed to set session:', error);
          return { success: false, error: error.message };
        }

        console.log('[AgencyModule] Session set successfully');
        return { success: true };
      } catch (error: any) {
        console.error('[AgencyModule] Exception setting session:', error);
        return { success: false, error: error.message };
      }
    });
    ...
    ipcMain.handle('agency:getUserProfile', async () => {
      try {
        console.log('[AgencyModule] getUserProfile called');

        const { data: { session } } = await supabase.auth.getSession();
        const { data: { user } } = await supabase.auth.getUser();

        console.log('[AgencyModule] Auth state:', {
          hasSession: !!session,
          hasUser: !!user,
          userId: user?.id,
        });
        ...
        if (!user) {
          console.error('[AgencyModule] No user found');
          return {
            success: false,
            error: '로그인이 필요합니다',
          };
        ...
        console.log('[AgencyModule] Fetching profile for user:', user.id);
        ...
        if (error) {
          console.log('[AgencyModule] Profile query error:', error);
          // 프로필이 없는 경우 (첫 로그인)
          if (error.code === 'PGRST116') {
            console.log('[AgencyModule] No profile found (first login)');
            return {
              success: true,
              data: null,
            };
        ...
        console.log('[AgencyModule] Profile found:', profile);
        ...
        console.error('[AgencyModule] Failed to get user profile:', error);
        return {
          success: false,
          error: error.message,
        };
    ...
    ipcMain.handle('agency:create', async (_, agencyName: string, subscriptionMonths: number = 1) => {
      try {
        ...
        console.log('[AgencyModule] agency:create - Auth check:', {
          hasSession: !!session,
          hasUser: !!user,
          userId: user?.id,
          authError
    ...
    console.log('[AgencyModule] Agency handlers registered');
  }
}
